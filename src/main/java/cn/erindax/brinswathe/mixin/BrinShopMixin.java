...
public abstract class BrinShopMixin {
    ...
    private void brinTryBuy(int index, Operation<Void> original) {
        ...
        if (index >= 0 && index < entries.size()) {
            ...
            if (BrinShopAccess.isCreditPurchase(entry)) {
            ...
            if (game.isRole(this.player, BrinRoles.BERSERKER)
                ...
                boolean skipOwnedCheck = BrinShopAccess.ignoresOwnedCheck(entry);
                purchased = this.balance >= entry.price()
                    && !this.player.getCooldowns().isOnCooldown(entry.stack().getItem())
                    && (skipOwnedCheck || !this.player.getInventory().contains(entry.stack()))
                    && entry.onBuy(this.player);
            }
            ...
            if (purchased) {
                ...
                this.player.displayClientMessage(
                    Component.literal("购买失败").withStyle(ChatFormatting.RED),
                    true
                );
